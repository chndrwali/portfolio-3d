...
import { techIcons } from '@/utils/constant/techIcon';
    ...
    <section className="text-black-primary">
      ...
      <div className="mx-auto mb-12 mt-8 flex flex-col items-center justify-center md:gap-10 lg:flex-row">
        <div className="relative aspect-auto h-60 w-60 items-baseline justify-center rounded-3xl border-4 border-black-primary bg-yellow-primary object-cover shadow-image-card shadow-black-primary md:h-96 md:w-1/2 xl:w-96">
          ...
          <Image src={'/me.png'} alt="Picture of the author" width={1000} height={1000} className="aspect-auto h-full w-full rounded-lg object-cover px-8" />
        </div>
        <p className="mt-5 text-justify text-base text-secondary-text md:mt-0 lg:w-1/2 xl:text-lg">
          I am graduate with a Bachelor{"'"}s Degree in <b>Informatics Engineering</b> from Universitas Nasional Pasim. I have a strong understanding of programming languages and web development, along with hands-on experience in{' '}
          <b>software development</b> projects. My current focus is on deepening my knowledge in the field of <b>web development</b>.
          <br />
          <br />I have hands-on experience with <b>ReactJS</b> and <b>NextJS</b> for the frontend. and using <b>ExpressJS</b> for backend development. Currently, i am also learning about <b>Laravel</b> for backend development
        </p>
      </div>
        ...
        {techIcons.map((data, index) => {
          ...
          return (
            <motion.div
              ...
              viewport={{ once: true }}
              custom={index}
              className={twMerge(`flex h-24 w-24 flex-col items-center justify-center gap-y-2 rounded-2xl border-2 bg-white duration-150 hover:scale-105 hover:border-black-primary hover:shadow-button-card md:h-36 md:w-36 md:p-4`)}
            >
              <Icons className={twMerge(`text-4xl text-black-primary md:text-6xl`)} />
